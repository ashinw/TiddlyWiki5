...
var temp = (function(options) {
	...
	var Diagram = funcs.Diagram = function Diagram(items) {
		...
		if(!(this.items[0] instanceof Start)) {
			var startCfg = {};
			if (Diagram.config.start)
				startCfg.type = Diagram.config.start;
			if (Diagram.config.startLabel)
				startCfg.label = Diagram.config.startLabel;			
			this.items.unshift(new Start(startCfg));
		}
		if(!(this.items[this.items.length-1] instanceof End)) {
			var endCfg = {};
			if (Diagram.config.end)
				endCfg.type = Diagram.config.end;
			this.items.push(new End(endCfg));
		}
	...
	function Link(item, twOptions) {
		if (!(this instanceof Link)) return new Link(item, twOptions);
		FakeSVG.call(this, 'a', twOptions);
		this.item = item;
	...
	function generate(rrScript) {
		var ret = eval(rrScript); 
		return ret;
	}
